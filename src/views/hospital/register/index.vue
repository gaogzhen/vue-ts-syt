...
<template>
  <div class="register">
    ...
    <div class="content">
      ...
      <div class="right">
        <div class="rule-register">挂号规则</div>
        <div class="time">
          预约周期: {{ detailStore.hospitalInfo.bookingRule?.cycle }}天 放号时间:{{ detailStore.hospitalInfo.bookingRule?.releaseTime }} 停诊时间:{{ detailStore.hospitalInfo.bookingRule?.stopTime }}
        </div>
        <div class="address">
          具体地址: {{ detailStore.hospitalInfo.hospital?.param?.fullAddress }}
        </div>
        <div class="route">
          具体路线: {{ detailStore.hospitalInfo.hospital?.route }}
        </div>
        <div class="withdrawal-time">
          退号时间: 就诊前一工作日{{ detailStore.hospitalInfo.bookingRule?.quitTime }}前取消
        </div> 
        <div class="rule-reservation">预约挂号规则</div>
        <div class="rule-info" v-for="(item,index) in detailStore.hospitalInfo.bookingRule?.rule" :key="index">
          {{ item }}
        </div>
      </div>
    </div>
...
.register{
  ...
  .content{
    ...
    .right{
      ...
      .time,.address,.route,.withdrawal-time,.rule-info{
        margin-top: 10px;
        color: #7f7f7f;
      }
      .rule-reservation{
